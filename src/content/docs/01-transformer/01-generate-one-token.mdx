...
<ChapterMeta
  ...
  difficulty={2}
  readingTime={28}
  prev={{ href: "/00-introduction/", title: "这本书讲什么" }}
...
推理时 $x_{<t}$ 已知，只要估计 $p(x_t\mid x_{<t})$。
...
$\tau$ 是温度。贪心则取 $\arg\max z_i$，不必经过采样。
...
flowchart LR
  ...
  subgraph In["输入"]
    Text(["文本 Prompt"]):::client --> Tok["Tokenizer"]:::control
    Tok --> Ids["token ids"]:::control
  end

  subgraph Model["模型"]
    Emb["Embedding"]:::compute
    Blocks["L × Transformer Block"]:::compute
    Last["取最后位置 hidden"]:::compute
    Head["lm_head"]:::compute
  end

  subgraph Out["采样"]
    Logits["logits · 长度 = 词表"]:::sample
    Sam["Sampler"]:::sample
    New(["new token id"]):::sample
  end

  Ids --> Emb --> Blocks --> Last --> Head --> Logits --> Sam --> New
  New -.-> Ids
```
...
sequenceDiagram
  autonumber
  actor P as Prompt ids
  participant M as Model forward
  participant S as Sampler

  P->>M: 全部 prompt
  M->>S: logits @ last prompt position
  S-->>P: token t1
  P->>M: prompt + t1
  M->>S: logits @ t1
  S-->>P: token t2
