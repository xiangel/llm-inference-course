...
**模型。** Su et al., 2021, RoFormer：对位置 $m$ 的向量乘旋转矩阵 $R_m$。相对位置出现在：
...
频率 $\theta_i=\theta^{-2i/d}$。Llama 2 常用 $\theta=10^4$；Llama 3.1 把 base 提到 $5\times10^5$ 以服务长上下文（Llama 3 论文 Table 3）。
...
flowchart LR
  ...
  subgraph Pos["位置"]
    QK(["Q, K"]):::compute --> Rot["RoPE 旋转"]:::compute
    Rot --> Attn["Attention"]:::compute
    Attn --> Layers["其余层"]:::compute
  end

  subgraph Sample["采样"]
    Z["logits z"]:::sample
    T["÷ temperature"]:::sample
    K["top-k 截断"]:::sample
    P["top-p 截断"]:::sample
    SM["softmax"]:::sample
    Draw(["sample / argmax"]):::sample
    Z --> T --> K --> P --> SM --> Draw
  end
...
频率（`head_dim=4`, $\theta=10000$）：

$$
\theta_0=10000^{0}=1,\quad \theta_1=10000^{-2/4}=0.01
$$
...
与 `examples/ch01/rope.py` 的 `q_paper[1]` 一致。

温度：$\tau\to0$ 时 softmax 趋近 one-hot（数值上还受浮点限制）；$\tau>1$ 变平。

Nucleus（Holtzman et al. 的思想；HF 文档称 top-p）：按概率降序取最小集合，使累计质量 $\ge p$。
