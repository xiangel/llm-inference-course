...
**数学。** Vaswani et al. 2017, §3.2.1 Scaled Dot-Product Attention：

$$
\mathrm{Attention}(Q,K,V)=\mathrm{softmax}\!\left(\frac{QK^{\top}}{\sqrt{d_k}}+M\right)V
$$

因果掩码 $M$：不允许的位置为 $-\infty$，softmax 后为 0。论文原话：illegal connections 在 softmax 前被设为 $-\infty$。
...
flowchart TB
  ...
  subgraph Attend["因果注意力"]
    S["scores = QKᵀ / √d"]:::compute
    M["+ 因果 mask"]:::compute
    SM["softmax"]:::compute
    O["weights × V"]:::compute
  end
...
$\sqrt{d_k}=\sqrt{2}\approx1.414$，裸分数 $QK^{\top}/\sqrt{d_k}$：
