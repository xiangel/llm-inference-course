...
flowchart TB
  ...
  X(["h：每个 token 一条向量"]):::memory

  subgraph AttnPath["注意力子层"]
    N1["RMSNorm"]:::compute
    QKV["Q / K / V 投影"]:::compute
    RoPE["RoPE on Q, K"]:::compute
    Attn["因果 Attention"]:::compute
    O["W_O"]:::compute
  end

  R1(["残差 +"]):::sample

  subgraph FfnPath["前馈子层"]
    N2["RMSNorm"]:::compute
    FFN["FFN / SwiGLU"]:::compute
  end

  R2(["残差 +"]):::sample
  Next(["下一层或 lm_head"]):::memory

  X --> N1 --> QKV --> RoPE --> Attn --> O --> R1
  X --> R1
  R1 --> N2 --> FFN --> R2
  R1 --> R2 --> Next
```
...
```mermaid
flowchart LR
  classDef enc fill:#dcecf5,stroke:#4ea1d3,color:#1a3344,stroke-width:1.5px
  classDef dec fill:#c4923a,stroke:#8a6a2e,color:#171410,stroke-width:1.6px
  classDef ed fill:#ead89a,stroke:#c4923a,color:#2c2416,stroke-width:1.5px

  subgraph Enc["Encoder-only"]
    direction TB
    E1["BERT 可看全句"]:::enc --> T1["分类 / 检索"]:::enc
  end

  subgraph Dec["Decoder-only"]
    direction TB
    D1["GPT / Llama 只看前文"]:::dec --> T2["聊天 / 续写"]:::dec
  end

  subgraph ED["Encoder-Decoder"]
    direction TB
    ED1["T5"]:::ed --> T3["翻译 / 摘要"]:::ed
  end
```
...
RMSNorm（Zhang & Sennrich 思想；Llama 使用）：
...
SwiGLU FFN（Llama 3 Table 3，8B）：

$$
\mathrm{FFN}(x)=W_2\bigl(\mathrm{SiLU}(W_1 x)\odot (W_3 x)\bigr)
$$

形状：`hidden=4096`，`intermediate=14336`。一次 FFN 的 GEMM FLOPs（忽略激活）：

$$
2 \times B \times S \times (2 \times 4096 \times 14336 + 14336 \times 4096)
$$

代入 $B=4,S=1024$：

- $W_1,W_3$：各 $2\times4\times1024\times4096\times14336 = 4.810\times 10^{11}$
- $W_2$：$2\times4\times1024\times14336\times4096 = 4.810\times 10^{11}$
- 合计约 $1.443\times 10^{12}$ FLOPs **每层 FFN**
...
再算一遍：教学尺寸 Attention 权重 256 MiB。若 `batch=1,S=4096`：
