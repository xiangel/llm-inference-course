...
**数学。** 扩容不是线性的。$P$ 张卡做张量并行时，单步 Decode 往往要一次 AllReduce。若通信时间 $T_{\text{comm}}$ 接近计算时间 $T_{\text{comp}}$，再加卡会变慢。
...
flowchart TB
  ...
  T(["1 · Token<br/>logits → sample"]):::client
  S["2 · Sequence<br/>Prefill / Decode / KV"]:::control
  E["3 · Engine<br/>Scheduler + BlockManager"]:::control
  MG["4 · Multi-GPU<br/>TP / PP / DP / EP"]:::compute
  DC["5 · Datacenter<br/>Gateway · Router · GPU Pool"]:::compute
  WK{{"6 · 万卡集群<br/>Global Scheduler · PD · KV Infra"}}:::device
...
flowchart TB
  ...
  U(["用户流量"]):::client --> GG["Global Gateway"]:::control
  GG --> GR["Global Router"]:::control

  subgraph Compute["计算池"]
    PC["Prefill Cluster"]:::compute
    DC2["Decode Cluster"]:::compute
    MC["其他 Model Cluster"]:::compute
  end

  Net{{"NVLink / IB / NCCL"}}:::device
...
$$
2 \times 80 \times 8 \times 128 \times 1024 \times 4 \times 2 / 1024^{3} = 1.25\ \text{GiB}
$$
