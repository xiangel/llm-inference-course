...
```mermaid
flowchart TB
  P0["第 0 篇 地图"] --> P1["第一篇 模型计算"]
  P1 --> P2["第二篇 Prefill/Decode/KV"]
  P2 --> P3["第三篇 GPU / FlashAttention"]
  P2 --> P4["第四篇 Scheduler / Paged KV"]
  P3 --> P5["第五篇 Mini-vLLM"]
  P4 --> P5
  P5 --> P6["第六篇 vLLM V1"]
  P6 --> P7["第七篇 Kernel"]
  P6 --> P8["第八篇 Quantization"]
  P6 --> P9["第九篇 Multi-GPU"]
  P7 --> P10["第十篇 引擎对比"]
  P8 --> P10
  P9 --> P10
  P10 --> P11["第十一篇 高级技术"]
  P11 --> P12["第十二篇 Serving 平台"]
  P12 --> P13["第十三篇 性能工程"]
  P13 --> P14["第十四篇 容量与成本"]
  P14 --> P15["第十五篇 万卡"]
  P15 --> P16["第十六篇 总项目"]
...
| 符号 | 含义 | 教学取值 | 对应真实配置 |
| --- | --- | --- | --- |
| $B$ | batch，同时计算的序列数 | 4 | 引擎 running batch |
| $S$ | 序列长度 | 1024 token | prompt + 已生成 |
| $d_{\text{model}}$ | 隐状态宽度 | 4096 | `hidden_size`；Llama 3.1 8B 同为 4096 |
| $H$ | Query 头数 | 32 | `num_attention_heads` |
| $d_h$ | 每头维度 | $4096/32=128$ | 必须整除 |

以及一套真实对照（[Llama 3 Table 3](https://arxiv.org/abs/2407.21783)）：

$$
\text{Llama 3.1 70B: } L=80,\ H_q=64,\ H_{kv}=8,\ d_{\text{model}}=8192
$$

| 符号 | 含义 | 70B 取值 | Hugging Face 字段 |
| --- | --- | --- | --- |
| $L$ | 层数 | 80 | `num_hidden_layers` |
| $H_q$ | Query 头数 | 64 | `num_attention_heads` |
| $H_{kv}$ | Key/Value 头数 | 8 | `num_key_value_heads`（GQA） |
| $d_{\text{model}}$ | 隐状态宽度 | 8192 | `hidden_size` |
