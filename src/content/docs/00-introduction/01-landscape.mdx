...
flowchart LR
  ...
  subgraph Traffic["流量面"]
    User(["用户"]):::client
    GlobalGW["Global Gateway"]:::control
    Router["Model Router"]:::control
  end

  subgraph Clusters["模型集群"]
    C1["70B Cluster"]:::compute
    C2["8B Cluster"]:::compute
    C3["Prefill Cluster"]:::compute
  end

  subgraph Pool["算力"]
    GPU1{{"GPU Pool"}}:::device
  end

  User --> GlobalGW --> Router
  Router --> C1 & C2 & C3
  C1 & C2 & C3 --> GPU1
```
